...
function App() {
  ...
  const validate=()=>{
   if(
    ...
    setEmptyerror("Sorry , All fields are mandatory")
    setUsername("")
    return false
   }
   ...
   if(!gender.match(/Male|Female|Other/i)){
    setGendererror("Please Select Any one from List")
    setUsername("")
    return false
   }
  ...
  return (
    <div className='App'>
     ...
     <form onSubmit={handleSubmit}>
      ...
      <ul>
        <li>Must contain one Upper Letter</li>
        <li>Must contain one Lower Letter</li>
     ...
     <h2>{username ? "Thanks For Registring  " + username : ""}</h2>
    </div>
  );
}
